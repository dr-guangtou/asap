...
def kl_divergence(pdf1, pdf2):
    ...
    """
    # no contribution from bins where there is no density in the target PDF
    idx = (pdf1 != 0)
...
def convergence_check(chain,
    ...
    niter_check_start = (2 * convergence_chunks)
    ncheck = (
        np.floor((niter - niter_check_start) /
                 float(convergence_check_interval)
                 ).astype(int) + 1
        )
    ...
    for n in range(ncheck):
        for i in range(npars):
            ...
            pdf_early, bins = make_kl_bins(early_test_chain,
                                           nbins=convergence_nhist)
    ...
    convergence_flag = find_subsequence(
        [True] * convergence_stable_points_criteria,
