...
def delta_sigma(galaxies, particles, particle_masses,
                downsampling_factor, rp_bins, period,
                cosmology=default_cosmology, num_threads=1,
                approx_cell1_size=None, approx_cell2_size=None):
    r"""
    Calculate :math:`\Delta\Sigma(r_p)`, the galaxy-galaxy lensing signal
    as a function of projected distance.

    The `delta_sigma` function calculates :math:`\Delta\Sigma(r_p)` by
    calculating the excess surface density of particles in cylinders
    surrounding the input galaxies.
    ...
    galaxies : array_like
        ...
        Length units are comoving and assumed to be in Mpc/h, here and
        throughout Halotools.

        See the :ref:`mock_obs_pos_formatting` documentation page for
        instructions on how to transform your coordinate position arrays into
        the format accepted by the ``galaxies`` and ``particles`` arguments.

    particles : array_like
        Numpy array of shape (num_ptcl, 3) containing 3-d positions of
        particles.
    ...
    particle_masses : float or ndarray
        Float or array storing the mass of each particle in units of Msun with
        h=1 units.
    ...
    num_threads : int, optional
        Number of threads to use in calculation, where parallelization is
        performed using the python ``multiprocessing`` module.
        Default is 1 for a purely serial calculation, in which case a
        multiprocessing Pool object will never be instantiated.
        A string 'max' may be used to indicate that the pair counters should
    ...
    -------
    rp_mids : array_like
        Numpy array of shape (num_rbins-1, ) storing the projected radii at
        which `Delta_Sigma` has been evaluated.
    ...
    into the shape of the ndarray expected by the
        `~halotools.mock_observables.delta_sigma`
    function. We transform our *x, y, z* points into the array shape used by
    the pair-counter by taking the transpose of the result of `numpy.vstack`.
    This boilerplate transformation is used throughout the
        `~halotools.mock_observables` sub-package:
    ...
    Whether or not you perform additional downsampling, you will need to
    account for the fact that you are not using the entire snapshot of
    particles by providing the ``downsampling_factor`` argument:
    ...
    >>> rp_mids, ds = delta_sigma(galaxies, particles, particle_masses,
        downsampling_factor, rp_bins, period)
    ...
    *physical* units of :math:`M_{\odot} / {\rm pc}^2` using the value of
    ...
    The code shown above demonstrates how to calculate :math:`\Delta\Sigma` via
    the excess surface density of mass using the z-axis as the axis of
    projection.
    However, it may be useful to project along the other Cartesian axes, for
    example to help beat down sample variance.
    While the `delta_sigma` function is written to always use the "third"
    dimension as the projection axis, you can easily hack the code to project
    along, say, the y-axis by simply transposing your y- and z-coordinates when
    you pack them into a 2-d array:

    >>> particles = np.vstack((px, pz, py)).T
    >>> galaxies = np.vstack((x, z, y)).T

    Using the above ``particles`` and ``galaxies`` and otherwise calling the
    `delta_sigma` function as normal will instead calculate the surface mass
    density by projecting along the y-axis.
    ...
    one_plus_mean_sigma_inside_rp = mean_sigma_comoving * (
        ...
        expected_mass_in_random_stack_of_cylinders)
    ...
    one_plus_mean_sigma_inside_rp_interp = log_interp(
        one_plus_mean_sigma_inside_rp, rp_bins, rp_mids)

    excess_surface_density = (one_plus_mean_sigma_inside_rp_interp -
                              one_plus_sigma)
...
def delta_sigma_from_precomputed_pairs(galaxies, mass_enclosed_per_galaxy,
    ...
    mass_enclosed_per_galaxy : array_like
        Numpy array of shape (num_gal, num_rp_bins+1) storing the mass enclosed
        inside each of the cylinders defined by the input ``rp_bins``.
    ...
    This boilerplate transformation
    ...
    >>> mass_encl = total_mass_enclosed_per_cylinder(galaxies, particles,
    ...
    >>> rp, ds = delta_sigma_from_precomputed_pairs(cens, mass_encl_cens,
    ...
    *physical* units of :math:`M_{\odot} / {\rm pc}^2` using the value of
    ...
    one_plus_sigma = mean_sigma_comoving * (
        total_mass_in_stack_of_annuli /
        expected_mass_in_random_stack_of_annuli)
...
def _delta_sigma_precomputed_process_args(galaxies, mass_enclosed_per_galaxy,
                                          rp_bins, period):
    """Delta sigma using precomputed results."""
    period, PBCs = get_period(period)
    galaxies = enforce_sample_has_correct_shape(galaxies)
    enforce_sample_respects_pbcs(galaxies[:, 0],
                                 galaxies[:, 1],
                                 galaxies[:, 2], period)
    rp_bins = get_separation_bins_array(rp_bins)

    num_gals, num_rp_bins = galaxies.shape[0], rp_bins.shape[0]
    msg = "Shape of ``mass_enclosed_per_galaxy`` must be ({0}, {1})".format(
...
def _delta_sigma_process_args(galaxies, particles, masses, downsampling_factor,
    ...
    msg = "downsampling_factor = {0} should > 1".format(downsampling_factor)
    ...
    enforce_sample_respects_pbcs(particles[:, 0],
                                 particles[:, 1],
                                 particles[:, 2], period)
    ...
    return (galaxies, particles, masses, downsampling_factor,
            rp_bins, period, num_threads, PBCs)
